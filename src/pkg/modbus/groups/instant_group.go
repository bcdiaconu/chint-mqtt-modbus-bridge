...
const instantRegisterDataSize = 4 // Each value is 4 bytes (float32)
...
func NewInstantGroup(registers []config.Register, commands map[string]modbus.ModbusCommand, keys []string) *InstantGroup {
	return &InstantGroup{
		Registers:       registers,
...
func (g *InstantGroup) Execute(ctx context.Context, gateway modbus.Gateway, slaveID uint8) ([]byte, error) {
...
func (g *InstantGroup) ParseResults(rawData []byte) (map[string]float64, error) {
	...
	for i, reg := range g.Registers {
		...
		if offset+instantRegisterDataSize > len(rawData) {
			return nil, fmt.Errorf("insufficient data for register %s at offset %d", reg.Name, offset)
		}
		dataSlice := rawData[offset : offset+instantRegisterDataSize]
